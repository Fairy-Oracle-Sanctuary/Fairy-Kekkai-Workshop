...
template <class T>
inline T clamp_val(T x, T min, T max)
{
    if (x > max)
        return max;
    if (x < min)
...
float boxScoreFast(const TextBox &boxes, const cv::Mat &pred)
{
    ...
    int minX = clamp_val(int(std::floor(left)), 0, width - 1);
    int maxX = clamp_val(int(std::ceil(right)), 0, width - 1);
    int minY = clamp_val(int(std::floor(top)), 0, height - 1);
    int maxY = clamp_val(int(std::ceil(bottom)), 0, height - 1);
...
TextLine CrnnNet::scoreToTextLine(const std::vector<float> &outputData, size_t h, size_t w)
{
    ...
    for (size_t i = 0; i < h; i++)
    {
        ...
        if (maxIndex > 0 && maxIndex < keySize && (!(i > 0 && maxIndex == lastIndex)))
        {
            ...
            textLine.text.append(keys[maxIndex]);
        }
        lastIndex = maxIndex;
    }
    if (!scores.empty()) {
        ...
        textLine.confidence = sum / scores.size();
    }
    return textLine;
}
