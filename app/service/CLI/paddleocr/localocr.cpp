...
DECLARE_API void OcrDetect(OcrLite *pOcrObj, const cv::Mat *mat,
                           Directional mode, void (*cb)(float, float, float, float, float, float, float, float, float, const char *), void (*cb2)(const char *))
{
    if (!pOcrObj)
        return;
    ...
    try
    {
        ...
        for (auto item : result)
        {
            cb(item.first[0].x, item.first[0].y,
               ...
               item.second.confidence,
               item.second.text.c_str());
        }
    }
    ...
    {
        cb2("OcrDetect: unknown exception (SEH/access violation)");
    }
}
