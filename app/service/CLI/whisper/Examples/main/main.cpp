...
static HRESULT loadWhisperModel(const wchar_t *path, const std::wstring &gpu,
                                iModel **pp) {
  using namespace Whisper;
  sModelSetup setup;
...
std::string to_timestamp(sTimeSpan ts, bool comma = false) {
  ...
  char buf[32];
  snprintf(buf, sizeof(buf), "%02d:%02d:%02d%s%03d", hr, min, sec,
           comma ? "," : ".", msec);
...
HRESULT __cdecl newSegmentCallback(iContext *context, uint32_t n_new,
                                   void *user_data) noexcept {
  ComLight::CComPtr<iTranscribeResult> results;
  CHECK(context->getResults(eResultFlags::Timestamps | eResultFlags::Tokens,
                            &results));
  ...
  for (uint32_t i = s0; i < length.countSegments; i++) {
    ...
    if (params.print_colors) {
      printf("[%s --> %s] %s ", to_timestamp(seg.time.begin).c_str(),
             to_timestamp(seg.time.end).c_str(), speaker.c_str());
      ...
      printf("[%s --> %s]  %s%s\n", to_timestamp(seg.time.begin).c_str(),
             to_timestamp(seg.time.end).c_str(), speaker.c_str(), seg.text);
  }
  return S_OK;
}

HRESULT __cdecl beginSegmentCallback(iContext *context,
                                     void *user_data) noexcept {
...
int wmain(int argc, wchar_t *argv[]) {
  // Whisper::dbgCompareTraces( LR"(C:\Temp\2remove\Whisper\ref.bin)",
  // LR"(C:\Temp\2remove\Whisper\gpu.bin )" ); return 0;

  // Set console to UTF-8 encoding to fix Chinese character display
  ...
  if (params.language != "auto" && !params.language.empty() &&
      Whisper::findLanguageKeyA(params.language.c_str()) == UINT_MAX) {
  ...
  for (const std::wstring &fname : params.fname_inp) {
    ...
    wparams.resetFlag(eFullParamsFlags::PrintRealtime |
                      eFullParamsFlags::PrintProgress);
    ...
    wparams.setFlag(eFullParamsFlags::TokenTimestamps,
                    params.output_wts || params.max_len > 0);
    ...
    // in this example, we do not abort the processing, but we could if the flag
    // is set to true the callback is called before every encoder run - if it
    // returns false, the processing is aborted
    ...
    if (STREAM_AUDIO && !wparams.flag(eFullParamsFlags::TokenTimestamps)) {
      ...
      // Token-level timestamps feature is not currently implemented when
      // streaming the audio When these timestamps are requested, fall back to
      // buffered mode.
