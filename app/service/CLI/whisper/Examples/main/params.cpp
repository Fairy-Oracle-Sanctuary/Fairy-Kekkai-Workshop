...
namespace {
const char *cstr(bool b) { return b ? "true" : "false"; }
} // namespace

void whisper_print_usage(int argc, wchar_t **argv,
  ...
  fprintf(stderr, "  -on N,    --offset-n N    [%-7d] segment index offset\n",
          params.offset_n);
  fprintf(stderr,
          "  -d  N,    --duration N    [%-7d] duration of audio to process in "
          "milliseconds\n",
          params.duration_ms);
  fprintf(stderr,
          "  -mc N,    --max-context N [%-7d] maximum number of text context "
          "tokens to store\n",
          params.max_context);
  fprintf(stderr,
          ...
          params.max_len);
  fprintf(stderr,
          "  -wt N,    --word-thold N  [%-7.2f] word timestamp probability "
          "threshold\n",
          params.word_thold);
  fprintf(stderr,
  ...
  fprintf(stderr,
          ...
          cstr(params.diarize));
  fprintf(stderr,
          "  -otxt,    --output-txt    [%-7s] output result in a text file\n",
  ...
  fprintf(stderr,
          "  -owts,    --output-words  [%-7s] output script for generating "
          "karaoke video\n",
  ...
  fprintf(stderr, "  -m FNAME, --model FNAME   [%-7S] model path\n",
          params.model.c_str());
  fprintf(stderr,
          "  -f FNAME, --file FNAME    [%-7s] path of the input audio file\n",
          "");
  fprintf(
      stderr,
...
bool whisper_params::parse(int argc, wchar_t *argv[]) {
  for (int i = 1; i < argc; i++) {
    ...
    } else if (arg == L"-p" || arg == L"--processors") {
      n_processors = std::stoul(argv[++i]);
    } else if (arg == L"-ot" || arg == L"--offset-t") {
      offset_t_ms = std::stoul(argv[++i]);
    } else if (arg == L"-on" || arg == L"--offset-n") {
      offset_n = std::stoul(argv[++i]);
    } else if (arg == L"-d" || arg == L"--duration") {
      duration_ms = std::stoul(argv[++i]);
    } else if (arg == L"-mc" || arg == L"--max-context") {
      max_context = std::stoul(argv[++i]);
    } else if (arg == L"-ml" || arg == L"--max-len") {
      max_len = std::stoul(argv[++i]);
    } else if (arg == L"-wt" || arg == L"--word-thold") {
      word_thold = std::stof(argv[++i]);
    } else if (arg == L"-su" || arg == L"--speed-up") {
      speed_up = true;
    } else if (arg == L"-tr" || arg == L"--translate") {
      translate = true;
    } else if (arg == L"-di" || arg == L"--diarize") {
      diarize = true;
    } else if (arg == L"-otxt" || arg == L"--output-txt") {
      output_txt = true;
    } else if (arg == L"-ovtt" || arg == L"--output-vtt") {
      output_vtt = true;
    } else if (arg == L"-osrt" || arg == L"--output-srt") {
      output_srt = true;
    } else if (arg == L"-owts" || arg == L"--output-words") {
      output_wts = true;
    } else if (arg == L"-ps" || arg == L"--print-special") {
      print_special = true;
    } else if (arg == L"-nc" || arg == L"--no-colors") {
      print_colors = false;
    } else if (arg == L"-nt" || arg == L"--no-timestamps") {
      no_timestamps = true;
    } else if (arg == L"-l" || arg == L"--language") {
      language = utf8(argv[++i]);
    } else if (arg == L"-m" || arg == L"--model") {
      model = argv[++i];
    } else if (arg == L"-f" || arg == L"--file") {
      fname_inp.push_back(argv[++i]);
    } else if (arg == L"-gpu" || arg == L"--use-gpu") {
      // 如果下一个参数不以 '-' 开头，则读取为 GPU 名称；否则视为启用默认 GPU
